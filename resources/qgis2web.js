var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
          maxZoom: 18, minZoom: 12, projection: new ol.proj.Projection({
            code: 'EPSG:8908',
            //extent: [443216.250000, 1119941.750000, 469271.937500, 1135918.500000],
            units: 'm'})
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([442778, 1119801, 469646, 1136423], map.getSize());
...
//attribution
// Esperar a que el control exista
setTimeout(function () {

    var attributionDiv = document.getElementsByClassName('bottom-attribution')[0];

    if (attributionDiv) {

        // Vaciar completamente el contenido generado por OL
        attributionDiv.innerHTML = '';

        // Crear botón
        var btn = document.createElement('button');
        btn.id = 'btnCreditos';
        btn.innerText = 'Créditos';

        btn.style.background = '#024589';
        ...
        attributionDiv.appendChild(btn);
    }

}, 300);

document.addEventListener("click", function(e) {
    if (e.target && e.target.id === "btnCreditos") {
